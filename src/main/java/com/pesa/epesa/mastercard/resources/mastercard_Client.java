...
import com.pesa.epesa.mastercard.authenticator.Mastercard_Authenticator;

/**
 *
 * @author User
 */
public class mastercard_Client {
 final static Logger logger = Logger.getLogger(mastercard_Client.class);
    ...
    private SSLContext loadClientCertificate() throws KeyManagementException, UnrecoverableKeyException,NoSuchAlgorithmException, KeyStoreException, CertificateException, IOException {        
    SSLContext sslcontext = SSLContexts.custom().loadKeyMaterial(new File(Mastercard_Properties.getProperty(Property.CONSUMERKEY)),Mastercard_Properties.getProperty(Property.KEYSTOREPATH).toCharArray(),Mastercard_Properties.getProperty(Property.KEYPASSWORD).toCharArray()).build();
    return sslcontext;
    }
    ...
    NoSuchAlgorithmException, KeyStoreException, CertificateException, IOException {
        SSLConnectionSocketFactory sslSocketFactory = new SSLConnectionSocketFactory(loadClientCertificate(),new String[] { "TLSv1.2" }, null, SSLConnectionSocketFactory.getDefaultHostnameVerifier());
        return sslSocketFactory;
    }
    
    private HttpRequest createHttpRequest(Mastercard_MethodTypes methodType, String url) throws Exception {
    ...
    public CloseableHttpResponse doMutualAuthRequest(String path, String testInfo, String body, Mastercard_MethodTypes methodType, Map<String, String> headers) 
                    throws Exception {
       
        String url = Mastercard_Properties.getProperty(Property.END_POINT) + path;
        logRequestBody(url, testInfo, body);
        HttpRequest request = createHttpRequest(methodType, url);
        request.setHeader(HttpHeaders.CONTENT_TYPE, "application/json");
        request.setHeader(HttpHeaders.ACCEPT, "application/json");
        request.setHeader(HttpHeaders.AUTHORIZATION, Mastercard_Authenticator.getBasicAuthHeader());
        ...
        HttpHost host = new HttpHost(Mastercard_Properties.getProperty(Property.END_POINT));
        CloseableHttpResponse response = fetchMutualAuthHttpClient().execute((HttpUriRequest) request);
        logResponse(response);
        return response;
    }
    
    public CloseableHttpResponse doXPayTokenRequest(String baseUri, String resourcePath, String queryParams, String testInfo, String body, Mastercard_MethodTypes methodType, Map<String, String> headers)
                    throws Exception {
        String url = Mastercard_Properties.getProperty(Property.END_POINT) + baseUri + resourcePath + "?" + queryParams;
        logRequestBody(url, testInfo, body);
        
        String apikey = Mastercard_Properties.getProperty(Property.CONSUMERKEY);
